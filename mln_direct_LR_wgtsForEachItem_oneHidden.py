...
def learn(num_iter=20, ds=original_ds, step_size=1e-5, pregl=0, trace=True):
    global w0, iter, wgts_posr_h, wgts_negr_h, wgt_h_g, wgt_nh_g, phgr
    sse, sll = 0, 0
    prev_sll = float("inf")
    ...
    for i in range(num_iter):
        ...
        if prev_sll < sll:
            # step_size *= 0.5
            if trace:
                print("worsening step; step size", step_size)
                print("worsening step; w0", w0)
                print("worsening step; wgt_h_g", wgt_h_g)
                print("worsening step; wgt_nh_g", wgt_nh_g)
